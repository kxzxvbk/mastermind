...
from metrics import compute_go_metrics
...
def compute_metrics(pred):
    ...
    return compute_go_metrics(pred_str, label_str)
...
def prepare_dataset(tokenizer):
    test_data = load_dataset("data/go/task1", split='test')
    train_data = load_dataset("data/go/task1", split='train')

    test_data += load_dataset("data/go/task2", split='test')
    train_data += load_dataset("data/go/task2", split='train')

    test_data += load_dataset("data/go/task3", split='test')
    train_data += load_dataset("data/go/task3", split='train')
...
if __name__ == '__main__':
    ...
    peft_config = LoraConfig(task_type=TaskType.CAUSAL_LM, inference_mode=False, r=32, lora_alpha=64, lora_dropout=0.1)
    ...
    main(train_dataset=train_dataset, test_dataset=test_dataset, model=model,
         tokenizer=tokenizer, compute_metrics=compute_metrics, output_dir='./output_go')
